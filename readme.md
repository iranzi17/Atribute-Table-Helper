...
3. In the **Single File Upload** section choose **Use stored reference workbook**, pick the workbook + sheet, review the automatic schema preview, and continue the merge as usual. If your sheet combines data for multiple substations/regions, the UI now lets you pick a column and value (e.g., `Substation = Gikondo`) so only the relevant rows are merged.
